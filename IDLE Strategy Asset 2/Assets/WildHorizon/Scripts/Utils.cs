...
namespace IdleStrategyKit
{
    public class Utils
    {
        ...
        public static string PrettySeconds(int seconds)
        {
            ...
            if (t.Minutes > 0) res += (res.Length > 0 ? " " : "") + t.Minutes + "min";
        ...
        public static string TimeBySeconds(float seconds)
        {
            var ss = Convert.ToInt32(seconds % 60).ToString("00");
            var mm = (Math.Floor(seconds / 60) % 60).ToString("00");
            var hh = Math.Floor(seconds / 60 / 60).ToString("00");
            return (hh + ":" + mm + ":" + ss);
        }
